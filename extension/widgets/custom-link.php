...
class herz_custom_link_widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        $widget_ops = array(
            'classname'     =>  'herz_custom_link_widget',
            'description'   =>  esc_html__( 'A widget show custom link', 'herz' ),
        );

        parent::__construct( 'herz_custom_link_widget', 'Basic Theme: Custom Link', $widget_ops );
    ...
    function widget( $args, $instance ) {
        ...
        $cat_ids    =   !empty( $instance['select_cat'] ) ? $instance['select_cat'] : 0;

        if ( !empty( $cat_ids ) ) :

            $tax_query = array(
                array(
                    'taxonomy' => 'custom_link_cat',
                    'field'    => 'term_id',
                    'terms'    => array( $cat_ids ),
                ),
            );
        ...
        ?>
            ...
            <div class="custom-link-widget-warp">
                <?php
                ...
                ?>
                    <div class="item">
                        ...
                        <div class="item-content">
                            <h4 class="item-title">
                                <a href="<?php echo esc_url(rwmb_meta( 'herz_link_custom_link' )); ?>" title="<?php the_title() ?>">
                                    <?php the_title(); ?>
                                </a>
                            </h4>
                        </div>
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
                <?php esc_attr_e( 'Select Categories:', 'herz' ); ?>
            </label>

            <select id="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'select_cat' ) ); ?>" class="widefat">
                <option value="0">
                    <?php esc_html_e( 'Select Category','herz' ); ?>
                </option>
                <?php
                if ( !empty( $terms ) ) :
                ...
                ?>
                        <option value="<?php echo esc_attr( $term_item->term_id ); ?>" <?php selected( $select_cat, $term_item->term_id ); ?>>
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo $this->get_field_name('order') ?>" class="widefat">
                <option value="ASC" <?php echo ( $order == 'ASC' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'ASC', 'herz' ); ?>
                </option>

                <option value="DESC" <?php echo ( $order == 'DESC' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'DESC', 'herz' ); ?>
                </option>
            </select>
        </p>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>">
                <?php esc_html_e( 'Number of posts to show:', 'herz' ); ?>
            </label>
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['select_cat'] =   (int) $new_instance['select_cat'];
...
// Register widget
function herz_custom_link_widget_register() {
    register_widget( 'herz_custom_link_widget' );
}

add_action( 'widgets_init', 'herz_custom_link_widget_register' );